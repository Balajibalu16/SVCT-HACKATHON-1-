module.exports = {
  analyze: (state) => {
    // Hackathon Logic Engine
    const text = `${state.main} ${state.additional}`.toLowerCase();
    ...
    let result = {
      ...
      matched_symptoms: [state.main],
      reasoning: 'Symptoms do not currently match severe acute patterns. Standard observation recommended.'
    };

    // Emergency Block (Red)
    if (text.includes('chest') || text.includes('breathe') || text.includes('breathing') || text.includes('heart')) {
       return {
    ...
    // High Urgency Block (Yellow)
    if ((text.includes('fever') || text.includes('hot')) && (text.includes('cough') || text.includes('throat'))) {
       result = {
    ...
    } else if (text.includes('headache') && state.severity.includes('severe')) {
       result = {
    ...
    } else if (text.includes('nausea') || text.includes('vomit') || text.includes('stomach')) {
       result = {
